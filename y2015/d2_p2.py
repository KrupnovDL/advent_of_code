...
for row in input_data:
    for_present = row.copy()
    for_present.pop(for_present.index(max(for_present)))
    for_present = sum([i+i for i in for_present])
    for_bow = reduce(lambda x, y: x*y, row)
    answer += for_present + for_bow
